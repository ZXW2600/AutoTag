...
class SuperImgShow:
    def __init__(self, windows_name, image):
        self.g_window_name = windows_name
        # 全局变量
        self.g_window_wh = [800, 600]  # 窗口宽高
        self.g_location_win = [0, 0]  # 相对于大图，窗口在图片中的位置
        self.location_win = [0, 0]  # 鼠标左键点击时，暂存g_location_win
        self.g_location_click, self. g_location_release = [
            0, 0], [0, 0]  # 相对于窗口，鼠标左键点击和释放的位置
        ...
        self.g_image_show = self.g_image_original[self.g_location_win[1]:self.g_location_win[1] +
                                                  self.g_window_wh[1], self.g_location_win[0]:self.g_location_win[0] + self.g_window_wh[0]]  # 实际显示的图片
        # 设置窗口
        cv2.namedWindow(self.g_window_name, cv2.WINDOW_NORMAL)
        # 设置窗口大小，只有当图片大于窗口时才能移动图片
        cv2.resizeWindow(self.g_window_name,
    ...
    def mouse(self, event, x, y, flags, parm):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:  # 左键点击
            ...
            # 窗口相对于图片的坐标，不能写成location_win = g_location_win
            location_win = [self.g_location_win[0], self.g_location_win[1]]
        # 按住左键拖曳
            ...
            if w1 < w2 and h1 < h2:  # 图片的宽高小于窗口宽高，无法移动
                ...
                show_wh = [w2, h1]
                self.g_location_win[0] = self.location_win[0] + \
                    self.g_location_click[0] - self.g_location_release[0]
            elif w1 < w2 and h1 >= h2:  # 图片的高度大于窗口的高度，可上下移动
                show_wh = [w1, h2]
                self.g_location_win[1] = self.location_win[1] + \
                    self.g_location_click[1] - self.g_location_release[1]
                ...
                self.g_location_win[1] = self.location_win[1] + \
                    self.g_location_click[1] - self.g_location_release[1]
            self.check_location([w1, h1], [w2, h2],
                                g_location_win)  # 矫正窗口在图片中的位置
            self.g_image_show = self.g_image_zoom[g_location_win[1]:self.g_location_win[1] +
                                                  show_wh[1], self.g_location_win[0]:self.g_location_win[0] + show_wh[0]]  # 实际显示的图片
        elif event == cv2.EVENT_MOUSEWHEEL:  # 滚轮
            z = self.g_zoom  # 缩放前的缩放倍数，用于计算缩放后窗口在图片中的位置
            self.g_zoom = self.count_zoom(
                flags, self.g_step, self.g_zoom)  # 计算缩放倍数
            w1, h1 = [int(self.g_image_original.shape[1] * self.g_zoom),
                      int(self.g_image_original.shape[0] * self.g_zoom)]  # 缩放图片的宽高
            w2, h2 = self.g_window_wh  # 窗口的宽高
            self.g_image_zoom = cv2.resize(
                self.g_image_original, (w1, h1), interpolation=cv2.INTER_AREA)  # 图片缩放
            show_wh = [0, 0]  # 实际显示图片的宽高
            ...
            g_location_win = [int((self.g_location_win[0] + x) * self.g_zoom / z - x),
                              int((self.g_location_win[1] + y) * self.g_zoom / z - y)]  # 缩放后，窗口在图片的位置
            self.check_location(
                [w1, h1], [w2, h2], g_location_win)  # 矫正窗口在图片中的位置
            # print(g_location_win, show_wh)
            self.g_image_show = self.g_image_zoom[self.g_location_win[1]:self.g_location_win[1] +
                                                  show_wh[1], self.g_location_win[0]:self.g_location_win[0] + show_wh[0]]  # 实际的显示图片
        cv2.imshow(self.g_window_name, self.g_image_show)
    ...
    def close(self):
        cv2.destroyWindow(self.g_window_name)

    def updateImage(self,image):
        self.g_image_original=np.copy(image)
        w1, h1 = [int(self.g_image_original.shape[1] * self.g_zoom),
                      int(self.g_image_original.shape[0] * self.g_zoom)]  # 缩放图片的宽高
        w2, h2 = self.g_window_wh  # 窗口的宽高
        show_wh = [0, 0]  # 实际显示图片的宽高
...
if __name__ == "__main__":
    image_original = cv2.imread("000.png")  # 原始图片，建议大于窗口宽高（800*600）
    demo = SuperImgShow("demo", image_original)
    image_new = cv2.imread("001.png")  # 原始图片，建议大于窗口宽高（800*600）
    cv2.waitKey()
    demo.updateImage(image_new)
    cv2.waitKey()
